...
print("=" * 80)
print("[exp0030] Tweedie損失を使用したLightGBM学習")
print("=" * 80)

# Optunaで最適化しないベースパラメータ
base_params = {
    "objective": "tweedie",  # [exp0030] Tweedie損失を使用
    "boosting_type": "gbdt",
    "random_state": SEED,
    "verbosity": -1,
    "force_col_wise": True,
    ...
    "metric": "rmse",  # [exp0030] 評価はRMSEで統一
}
...
optuna_search_space = {
    ...
    "tweedie_variance_power": (1.1, 1.9),  # [exp0030] Tweedie variance powerを追加
}

print("\n[exp0030] Tweedie損失について:")
print("  ゼロ過剰データ（約69%がxAG<0.1）に適した確率分布")
print("  tweedie_variance_power: 1.1～1.9 の範囲で最適化")
print("    1.0付近: Poisson分布")
print("    1.5付近: Compound Poisson-Gamma")
print("    2.0付近: Gamma分布")
...
def objective(trial):
    ...
    params.update({
        ...
        "tweedie_variance_power": trial.suggest_float("tweedie_variance_power", *optuna_search_space["tweedie_variance_power"]),  # [exp0030] 追加
    })
...
# [exp0030] LightGBM学習のパラメータを設定（Tweedie損失）
lgbm_params = {
    "objective": "tweedie",  # [exp0030] Tweedie損失を使用
    "boosting_type": "gbdt",
    "random_state": SEED,
...
# Optunaの最適パラメータをマージ（tweedie_variance_powerを含む）
lgbm_params.update(best_params)

print(f"\n[exp0030] 使用するパラメータ（Tweedie損失）:")
print(f"  objective: {lgbm_params['objective']}")
print(f"  tweedie_variance_power: {lgbm_params.get('tweedie_variance_power', 'N/A'):.4f}")
print(f"  その他: num_leaves={lgbm_params.get('num_leaves')}, lr={lgbm_params.get('learning_rate'):.4f}")
