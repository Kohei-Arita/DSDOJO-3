...
for action_group, action_names in xpass_action_groups.items():
    ...
    if train_subset.empty or train_subset["match_id"].nunique() < 2:
        ...
        relevant_actions.loc[action_subset.index, "xpass_action_group"] = action_group
    ...
    oof_preds = np.clip(oof_preds, 1e-6, 1 - 1e-6)

    if len(test_subset):
        test_preds = test_preds / len(models)
        test_preds = np.clip(test_preds, 1e-6, 1 - 1e-6)

    train_mean = float(train_subset["is_success"].mean()) if len(train_subset) else 0.5
    fallback = float(np.clip(train_mean, 1e-6, 1 - 1e-6))
...
if xpass_predictions_df.empty:
    ...
    group_stats = (
        train_actions.groupby("xpass_action_group")["xt_learned_start"]
        .agg(["mean", "std"])
        .rename(columns={"mean": "group_mean", "std": "group_std"})
        .reset_index()
    )
    group_stats["group_std"] = group_stats["group_std"].where(group_stats["group_std"] > 1e-6, global_start_std)

    mean_map = group_stats.set_index("xpass_action_group")["group_mean"].to_dict()
    std_map = group_stats.set_index("xpass_action_group")["group_std"].to_dict()

    xpass_predictions_df["fail_group_mean"] = xpass_predictions_df["xpass_action_group"].map(mean_map).fillna(global_start_mean)
    xpass_predictions_df["fail_group_std"] = xpass_predictions_df["xpass_action_group"].map(std_map).fillna(global_start_std)

    xpass_predictions_df["fail_component_scaled"] = xpass_predictions_df["fail_weight"] * (
        (xpass_predictions_df["xt_learned_start"] - xpass_predictions_df["fail_group_mean"]) / xpass_predictions_df["fail_group_std"]
    )
    ...
    for action_group in sorted(xpass_predictions_df["xpass_action_group"].dropna().unique()):
        ...
        def objective(trial):
            lam = trial.suggest_float("lambda", -1.0, 3.0)
...
submission_df.to_csv(f"{base_path}/host_baseline_0016_submissions.csv", index=False)
