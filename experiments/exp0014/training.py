...
if xpass_predictions_df.empty:
    ...
    for action_group in sorted(xpass_predictions_df["xpass_action_group"].dropna().unique()):
        ...
        def objective(trial):
            lam = trial.suggest_float("lambda", 0.0, 5.0)
            preds = type_df["success_sum"] - lam * type_df["fail_sum"]
            scores = []
    ...
    for action_group, lam_value in lambda_per_type.items():
        ...
        type_actions["edxt_value"] = type_actions["success_component"] - lam_value * type_actions["fail_component"]
        ...
        col_map = {
            ...
            "fail_sum": f"{action_group}_edxt_fail_sum",
        }
...
submission_df.to_csv(f"{base_path}/host_baseline_0014_submissions.csv", index=False)
