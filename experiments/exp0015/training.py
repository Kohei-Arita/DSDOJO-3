...
if xpass_predictions_df.empty:
    ...
    xpass_predictions_df["fail_component"] = (1.0 - xpass_predictions_df["xpass_prob"]) * xpass_predictions_df["xt_learned_start"]
    xpass_predictions_df["fail_component_raw"] = xpass_predictions_df["fail_component"].copy()
    ...
    fail_start_mean = float(train_actions["xt_learned_start"].mean()) if not train_actions.empty else 0.0
    fail_start_std = float(train_actions["xt_learned_start"].std(ddof=0)) if not train_actions.empty else 0.0
    if fail_start_std < 1e-6:
        fail_start_std = 1.0

    xpass_predictions_df["fail_component_scaled"] = xpass_predictions_df["fail_weight"] * (
        (xpass_predictions_df["xt_learned_start"] - fail_start_mean) / fail_start_std
    )
    ...
    for action_group in sorted(xpass_predictions_df["xpass_action_group"].dropna().unique()):
        ...
        def objective(trial):
            lam = trial.suggest_float("lambda", -0.5, 3.0)
        ...
        lambda_meta.append(
            {
                ...
                "fail_sum_scaled_mean": float(type_df["fail_sum_scaled"].mean()),
            }
        )

    if lambda_meta:
        display(pd.DataFrame(lambda_meta))
...
submission_df.to_csv(f"{base_path}/host_baseline_0015_submissions.csv", index=False)
