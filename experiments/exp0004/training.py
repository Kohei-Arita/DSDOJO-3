...
# モデル学習用データの作成
X_train = train_df[all_features + ["fold"]].copy()
y_train = train_df["xAG"].astype(float)
y_binary = (y_train > 0).astype(int)
X_test = test_df[all_features].copy()

positive_ratio = y_binary.mean()
print()
print(f"モデル学習用データ形状: X_train {X_train.shape}, y_train {y_train.shape}, X_test {X_test.shape}")
print(f"正例（xAG>0）の割合: {positive_ratio:.3f}")
...
print("回帰・分類ステージのパラメータ設定が完了しました。")
...
def save_training_run(
    ...
    optuna_info = optuna_summary.get("best_trial_number")
    if optuna_info is not None:
        log_lines.append(
            "  Optuna best trial: "
            f"{optuna_summary['best_trial_number']} (CV mean {optuna_summary.get('best_cv_value', float('nan')):.6f}, "
            f"fold1 RMSE {optuna_summary.get('fold1_val_rmse', float('nan')):.6f})"
        )
...
print("Optunaによる回帰パラメータチューニングを開始します...")


def objective(trial):
    params = regression_base_params.copy()
    params.update({
    ...
    cv_scores = []
    for fold in range(1, 4):  # Fold1~3で高速に探索
        trn_mask_cv = (train_df["fold"] != fold) & (y_train > 0)
        val_mask_cv = (train_df["fold"] == fold) & (y_train > 0)
        ...
        if X_tr_cv.empty or X_val_cv.empty:
            continue
...
best_lgbm_params = regression_base_params.copy()
best_lgbm_params.update(best_params)

# Fold1のポジティブデータでスコアを確認
pos_tr_mask = (train_df["fold"] != 1) & (y_train > 0)
pos_val_mask = (train_df["fold"] == 1) & (y_train > 0)

if pos_tr_mask.sum() > 0 and pos_val_mask.sum() > 0:
    X_tr = train_df.loc[pos_tr_mask, all_features].copy()
    y_tr = y_train.loc[pos_tr_mask].copy()
    X_val = train_df.loc[pos_val_mask, all_features].copy()
    y_val = y_train.loc[pos_val_mask].copy()
...
print("=== Optunaチューニング結果（回帰ステージ） ===")
print(f"最良Trial番号: {study.best_trial.number}")
print(f"平均CV RMSE (positive only): {study.best_value:.6f}")
print(f"Fold1 Positive Validation RMSE: {val_score:.6f}")
print("最適化されたパラメータ:")
...
print("分類→回帰の2段階モデルによる5-Fold Cross Validationを開始...")

classification_models = []
regression_models = []

oof_probs = np.zeros(len(X_train))
oof_regression = np.zeros(len(X_train))
oof_preds = np.zeros(len(X_train))

test_prob_sum = np.zeros(len(X_test))
test_reg_sum = np.zeros(len(X_test))
...
for fold in range(5):
    ...
    y_tr_reg = y_train.loc[trn_mask].copy()
    y_val_reg = y_train.loc[val_mask].copy()
    y_tr_clf = y_binary.loc[trn_mask].copy()
    y_val_clf = y_binary.loc[val_mask].copy()

    # 分類ステージ
    clf_train_data = lgb.Dataset(X_tr, label=y_tr_clf)
    clf_valid_data = lgb.Dataset(X_val, label=y_val_clf, reference=clf_train_data)
    clf_model = lgb.train(
        classification_params,
        clf_train_data,
        valid_sets=[clf_train_data, clf_valid_data],
        valid_names=["train", "val"],
        num_boost_round=1000,
        callbacks=[lgb.early_stopping(100), lgb.log_evaluation(0)],
    )
    classification_models.append(clf_model)

    val_probs = clf_model.predict(X_val, num_iteration=clf_model.best_iteration)
    oof_probs[val_mask] = val_probs

    val_logloss = log_loss(y_val_clf, np.clip(val_probs, 1e-6, 1 - 1e-6))
    classification_logloss.append(val_logloss)

    test_prob = clf_model.predict(X_test, num_iteration=clf_model.best_iteration)
    ...
    classification_feature_importance = pd.concat([classification_feature_importance, clf_importance], axis=0)

    # 回帰ステージ（正例データのみで学習）
    pos_tr_mask = y_tr_reg > 0
    reg_train_data = lgb.Dataset(X_tr.loc[pos_tr_mask], label=y_tr_reg.loc[pos_tr_mask])
    reg_valid_data = lgb.Dataset(X_val, label=y_val_reg, reference=reg_train_data)

    regression_params = best_lgbm_params.copy()
    regression_params["objective"] = weighted_rmse_obj

    reg_model = lgb.train(
        regression_params,
        reg_train_data,
        valid_sets=[reg_train_data, reg_valid_data],
        valid_names=["train_pos", "val_all"],
        feval=weighted_rmse_feval,
        num_boost_round=1000,
        callbacks=[lgb.early_stopping(100), lgb.log_evaluation(0)],
    )
    regression_models.append(reg_model)

    val_reg = reg_model.predict(X_val, num_iteration=reg_model.best_iteration)
    oof_regression[val_mask] = val_reg

    final_val_pred = val_probs * val_reg
    oof_preds[val_mask] = final_val_pred

    fold_score = weighted_rmse(y_val_reg, final_val_pred)
    cv_scores.append(fold_score)
    print(f"Fold {fold + 1} Weighted RMSE: {fold_score:.4f}")
    ...
    regression_feature_importance = pd.concat([regression_feature_importance, reg_importance], axis=0)

    test_reg = reg_model.predict(X_test, num_iteration=reg_model.best_iteration)
    test_reg_sum += test_reg
    test_preds_sum += test_prob * test_reg

cv_mean = float(np.mean(cv_scores))
cv_std = float(np.std(cv_scores))

print("=== Cross Validation Results (Two-Stage Model) ===")
print(f"CV Weighted RMSE: {cv_mean:.4f} (+/- {cv_std * 2:.4f})")
for i, score in enumerate(cv_scores):
    print(f"Fold {i + 1}: {score:.4f}")
print(f"分類ステージ平均Logloss: {np.mean(classification_logloss):.4f}")

oof_score = weighted_rmse(y_train, oof_preds)
print(f"OOF Weighted RMSE: {oof_score:.4f}")

test_prob_mean = test_prob_sum / 5
test_reg_mean = test_reg_sum / 5
test_preds = test_preds_sum / 5
...
feature_importance = regression_feature_importance.copy()
feature_importance_clf = classification_feature_importance.copy()
models = list(zip(classification_models, regression_models))
...
print(f"分類確率範囲: {test_prob_mean.min():.3f} 〜 {test_prob_mean.max():.3f}")
print(f"回帰ステージ予測範囲: {test_reg_mean.min():.3f} 〜 {test_reg_mean.max():.3f}")
print(f"最終xAG予測範囲: {test_preds.min():.3f} 〜 {test_preds.max():.3f}")
...
plt.title('Top 15 Feature Importance (Two-Stage Regression Model)')
...
submission_df.to_csv(f"{base_path}/host_baseline_005_submissions.csv", index=False)
