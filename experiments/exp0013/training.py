...
# インタラクション制約の設定（意味のある組み合わせのみ許可）
interaction_group_candidates = [
    ["xt_learned_positive_delta_sum", "xt_learned_positive_delta_mean", "possession_xt_delta_sum", "possession_xt_positive_sum"],
    ["pass_leads_to_shot", "pass_net_out_degree", "pass_net_receive_count"],
    ["player_id_target_enc", "Squad_target_enc", "Opponent_target_enc", "Squad_x_Opponent_target_enc"],
    ["pass_net_betweenness", "pass_net_clustering", "possession_actions_per_pos_mean"],
    ["pass_net_avg_distance", "possession_speed_ground_mean", "possession_speed_x_mean"],
]
interaction_constraints = []
for group in interaction_group_candidates:
    available = [feat for feat in group if feat in all_features]
    if len(available) >= 2:
        interaction_constraints.append(available)

print(f"インタラクション制約グループ数: {len(interaction_constraints)}")
lgbm_params = {
    "objective": weighted_rmse_obj,
    "boosting_type": "gbdt",
    "random_state": SEED,
...
submission_df.to_csv(f"{base_path}/host_baseline_0013_submissions.csv", index=False)
