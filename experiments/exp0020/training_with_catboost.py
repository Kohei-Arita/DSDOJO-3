...
# CatBoostの単調性制約形式: (feature_idx, direction)
# direction: 1=増加, -1=減少
catboost_monotone_constraints = [
    (catboost_features.index(feat), 1) 
    for feat in applied_catboost_monotone_features
]

print(f"単調性制約を適用: {len(applied_catboost_monotone_features)}個の特徴量")
print("  (LGBMの{0}個から削減し、モデルの多様性を確保)".format(len(applied_monotone_features)))
for feat in applied_catboost_monotone_features:
    print(f"  - {feat}")
