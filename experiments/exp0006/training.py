...
monotone_increase_features = [
    ...
    'progressive_distance_total',
    'progressive_distance_mean',
    ...
    # シュート・ゴール系（創造性の指標）
    'goal_count',  # ゴール数
    'pass_to_shot_count',  # パス→ショットの連鎖
    
    # 攻撃的ゾーン活動（前線でのプレー → xAG増加）
    'zone_attacking_count',  # 攻撃ゾーンでのアクション数
    'zone_attacking_ratio',  # 攻撃ゾーン比率
    
    # 攻撃的バランス
    'attacking_ratio',  # 攻撃アクション比率
]

# all_features内での各特徴量のインデックスを取得し、単調性ベクトルを構築
monotone_constraints = [0] * len(all_features)  # デフォルトは制約なし(0)
for feat in monotone_increase_features:
    if feat in all_features:
        idx = all_features.index(feat)
        monotone_constraints[idx] = 1  # 単調増加制約

print(f"\n単調性制約を適用: {sum(c == 1 for c in monotone_constraints)}個の特徴量")
...
submission_df.to_csv(f"{base_path}/host_baseline_006_submissions.csv", index=False)
